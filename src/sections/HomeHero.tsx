...
function ScrollBeatCard({ beat, index, progress, reduceMotion }: ScrollBeatCardProps) {
  ...
  return (
    <motion.article
      className="rounded-3xl border border-white/10 bg-void/46 p-5 shadow-2xl shadow-black/25 backdrop-blur-xl"
    ...
    >
      ...
      <h2 className="mt-4 text-xl font-semibold tracking-[-0.035em] text-platinum">
        {beat.title}
      </h2>
      <p className="mt-3 text-sm leading-6 text-white/58">{beat.body}</p>
    </motion.article>
  );
}
...
export function HomeHero() {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[420vh]" aria-labelledby="home-hero-title">
      <div className="sticky top-0 grid min-h-screen overflow-hidden px-6 py-8 sm:px-8 lg:px-12">
        <motion.div
          className="absolute inset-x-5 top-24 bottom-24 overflow-hidden border border-white/12 bg-graphite/80 shadow-[0_0_120px_rgba(90,108,255,0.22)] sm:inset-x-8 lg:inset-x-12"
          style={{
        ...
        <div className="relative z-10 flex min-h-full flex-col justify-between py-6">
          <motion.div
            className="mx-auto mt-16 max-w-6xl text-center sm:mt-20 lg:mt-24"
            style={{
          ...
          >
            <FadeIn>
              <p className="mb-6 text-xs font-medium uppercase tracking-[0.52em] text-white/48">
                Cinematic Site Lab
              </p>
              <h1
                id="home-hero-title"
                className="text-balance text-5xl font-semibold tracking-[-0.06em] text-platinum sm:text-7xl lg:text-8xl"
              >
                Scroll-shaped product stories with cinematic depth.
              </h1>
              <p className="mx-auto mt-8 max-w-2xl text-pretty text-base leading-8 text-white/64 sm:text-lg">
          ...
          <div className="mx-auto grid w-full max-w-6xl gap-5 pb-4 lg:grid-cols-[1fr_0.72fr] lg:items-end">
        ...
        <div className="absolute inset-x-6 bottom-5 z-20 h-px overflow-hidden rounded-full bg-white/12 sm:inset-x-8 lg:inset-x-12">
